...
export default function initOllamaEvents(i) {
    // Each player gets their own separate chat history — messages never mix
    ...
    i.bot.on('whisper', async (username, message) => {
        ...
        // Get or create this player's personal chat history
        if (!histories.has(username)) {
            histories.set(username, []);
            i.logger.info(`[OllamaAddon] New chat started for ${username}`);
        }
        ...
        try {
            ...
            // Store in THIS player's history only
            ...
            if (err.message.includes('ECONNREFUSED') || err.message.includes('fetch')) {
                whisperBack(username, `Ollama not responding — is it running?`);
            } else {
    ...
    i.logger.info(`[OllamaAddon] Ready! Allowed users: ${allowedUsers.join(', ')} | Model: ${OLLAMA_CONFIG.model}`);
}
